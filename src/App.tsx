...
export function App() {
  ...
  // ── Derived recommendation (synchronous) ──
  const recommendation: RecommendationResult = analyzeWatchHistoryAndRecommend(
    watchHistory,
    (SAMPLE_REELS[currentReelIndex] || SAMPLE_REELS[0]).id
  );
  ...
  }, [recommendation.output?.['RECOMMENDED TECH REEL']]);

  // ── Combine static + generated reels ──
  const allReels = [...SAMPLE_REELS, ...generatedReels];
  ...
  return (
    <div className="min-h-screen bg-[#080c14] text-slate-100 flex flex-col selection:bg-indigo-500/30 selection:text-indigo-200">
      ...
      <Navbar 
        ...
        onOpenTrapModal={() => setIsTrapModalOpen(true)}
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto p-4 sm:p-6 lg:p-8">
        ...
        {activePage === 'reels' && (
          <div className="relative flex flex-col items-center justify-center py-2 animate-in fade-in duration-300">
            ...
            <div className="w-full max-w-lg mb-4 flex items-center justify-between p-2.5 px-4 rounded-2xl glass-panel bg-slate-900/90 border border-indigo-500/30 shadow-lg">
              ...
              <div className="flex items-center space-x-1.5">
                <button
                  onClick={() => setIsReelsAiPillOpen(true)}
                  className="px-2.5 py-1 text-xs font-semibold rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white transition shadow-sm"
                >
                  Inspect AI
                </button>
              </div>
            ...
            {isReelsAiPillOpen && (
              <div 
                ...
                onClick={() => setIsReelsAiPillOpen(false)}
              >
                <div 
                ...
                >
                  ...
                  <div className="flex items-center justify-between pt-2">
                    ...
                    <button
                      onClick={() => setIsReelsAiPillOpen(false)}
                      className="px-3 py-1.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-xs text-white"
                    >
                      Close
                    </button>
                  </div>
          ...
          <div className="space-y-6 animate-in fade-in duration-300">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-4 rounded-2xl glass-panel bg-slate-900/80 border border-indigo-500/30">
              ...
              <button
                ...
                className="flex items-center space-x-1.5 px-3 py-1.5 rounded-xl bg-gradient-to-r from-rose-600 to-purple-600 hover:from-rose-500 hover:to-purple-500 text-white text-xs font-bold transition shadow-md shadow-rose-600/30"
              >
