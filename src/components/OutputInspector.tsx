...
CONFIDENCE: ${output['CONFIDENCE']}`;
  ...
  const getConfidenceBadgeClass = (conf: Confidence) => {
    switch (conf) {
      ...
      case 'Low': return 'bg-slate-500/20 text-slate-400 border-slate-500/40';
    }
  };

  return (
    <div className="glass-panel rounded-2xl p-4 sm:p-5 border border-indigo-500/30 shadow-2xl flex flex-col space-y-4">
      {/* Header with Copy Action Buttons */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-3 border-b border-slate-800">
        <div className="flex items-center space-x-2">
          <div className="p-2 rounded-xl bg-gradient-to-tr from-indigo-500 to-cyan-500 text-white shadow-md shadow-indigo-500/30">
        ...
        <div className="flex items-center space-x-2">
          <button
            ...
            className="flex items-center space-x-1.5 px-3 py-1.5 rounded-xl bg-indigo-600/80 hover:bg-indigo-500 text-white text-xs font-semibold transition shadow-sm"
          ...
          <button
            ...
            title="Copy as JSON"
          >
            {copiedJson ? <Check className="w-4 h-4 text-emerald-400" /> : <FileSpreadsheet className="w-4 h-4" />}
          </button>
        </div>
      </div>
